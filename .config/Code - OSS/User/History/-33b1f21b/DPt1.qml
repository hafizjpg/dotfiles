...
PanelWindow {
    ...
    function processQueue() {
        if (busy) return;
        if (Local.Shared.toastQueue.count === 0) {
            current = null;
            return;
        }
        busy = true;
    ...
    Connections {
        ...
        function onCountChanged() {
            if (Local.Shared.toastQueue.count > 0) Qt.callLater(toast.processQueue);
        }
    }
    ...
    SequentialAnimation {
        ...
        ScriptAction {
            script: {
                // only bother expanding if there's actually a body to show
        ...
        PauseAnimation { duration: 220 } // let the collapse animation finish before reuse
        ScriptAction {
            script: {
                toast.busy = false;
                toast.current = null;
        ...
        Rectangle {
            ...
            Item {
                ...
                Rectangle {
                    ...
                    anchors.top: parent.top
                    color: toast.current ? Local.Shared.urgencyColor(toast.current.urgency) + "33" : "transparent"
