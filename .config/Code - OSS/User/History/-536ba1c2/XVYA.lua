...
hl.layout.register("centered_master", {
    ...
    layout_msg = function(ctx, msg, param)
        ...
        -- Add this inside your layout_msg function in the script:
if msg == "make_master" then
    local active = hl.get_active_window()
    if active and active.address then
        ...
        -- Insert as primary window (index 1)
        table.insert(order, 1, active.address)
        
        -- Force untile / tile sync
        if active.floating then
            hl.dispatch("togglefloating", "address:" .. active.address)
        end
    end
    return
end
