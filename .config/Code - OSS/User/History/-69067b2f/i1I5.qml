    ...
    stderr: StdioCollector {
        onStreamFinished: {
            logProc.command = ["bash", "-c",
                "echo '--- ' $(date +%T) ' ---' >> /tmp/traydebug.log; " +
                "echo 'target: " + killProc.debugTarget + "' >> /tmp/traydebug.log; " +
                "echo 'stderr: " + (this.text || "(kosong)").replace(/'/g, "'\\''") + "' >> /tmp/traydebug.log"
            ]
...
MouseArea {
    ...
    onClicked: mouse => {
        ...
        logProc.command = ["bash", "-c",
            "echo 'CLICK button=" + (mouse.button === Qt.RightButton ? "RIGHT" : "LEFT")
            + " id=" + (item.id || "(no id)")
        ...
        if (mouse.button === Qt.RightButton) {
            ...
            if (target && target.length > 0) {
                killProc.debugTarget = target;
                killProc.command = ["bash", "-c",
                    "pkill -i -f " + JSON.stringify(target) + " || echo NOMATCH"];
            ...
            item.activate();
                            ...
                            MouseArea {
                                ...
                                acceptedButtons: Qt.LeftButton | Qt.RightButton
                                onClicked: mouse => {
                                    // Notifikasi ini WAJIB muncul kalau klik terdeteksi sama sekali.
                                    debugNotify.command = ["notify-send", "-t", "1500",
                                        "Tray click", (mouse.button === Qt.RightButton ? "RIGHT" : "LEFT")
                                        + " on " + (trayBtn.modelData.id || "(no id)")]
                                    debugNotify.running = true
