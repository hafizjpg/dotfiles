            ...
            MouseArea {
                ...
                onClicked: mouse => {
                    if (mouse.button === Qt.RightButton) {
                        ...
                        // Try to kill the owning process by matching its id/title via pkill.
                        // SystemTray items usually expose 'id' (desktop/app id) — fall back to title.
                        const target = item.id || item.title;
                        if (target && target.length > 0) {
                            killProc.command = ["bash", "-c", "pkill -if " + JSON.stringify(target)];
                            killProc.running = true;
                        } else if (item.hasMenu) {
                            // No reliable identifier — surface its own menu (often has Quit) instead.
                            item.display(bar, mouse.x, mouse.y);
                        }
                    } else {
